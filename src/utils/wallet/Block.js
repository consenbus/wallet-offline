...
module.exports = function() {
   ...
   *
   * @throw new Error(s An exception on invalid type
    ...
    switch (type) {
      case "send":
        ...
        data +=
          ...
          VERSION_USING +
          VERSION_MIN +
          functions.uint8_hex(blockID[type]) +
          EXTENSIONS;
        data += previous;
        data += destination;
        data += balance;
        ...
        blake.blake2bUpdate(context, functions.hex_uint8(previous));
        blake.blake2bUpdate(context, functions.hex_uint8(destination));
        blake.blake2bUpdate(context, functions.hex_uint8(balance));
        hash = functions.uint8_hex(blake.blake2bFinal(context));
        break;
      ...
      case "receive":
        ...
        context = blake.blake2bInit(32, null);
        blake.blake2bUpdate(context, functions.hex_uint8(previous));
        blake.blake2bUpdate(context, functions.hex_uint8(source));
        hash = functions.uint8_hex(blake.blake2bFinal(context));
        break;
      ...
      case "open":
        ...
        context = blake.blake2bInit(32, null);
        blake.blake2bUpdate(context, functions.hex_uint8(source));
        blake.blake2bUpdate(context, functions.hex_uint8(representative));
        blake.blake2bUpdate(context, functions.hex_uint8(account));
        hash = functions.uint8_hex(blake.blake2bFinal(context));
        break;
   ...
   * @throw new Error(s An exception on invalid block hash
   * @throw new Error(s An exception on invalid destination account
   * @throw new Error(s An exception on invalid balance
   */
  api.setSendParameters = function(
    previousBlockHash,
    destinationAccount,
    balanceRemaining
  ) {
    ...
    try {
      var pk = functions.keyFromAccount(destinationAccount);
    } catch (err) {
    ...
    balance = functions.dec2hex(balanceRemaining, 16);
    type = "send";
  };
   ...
   * @throw new Error(s An exception on invalid previousBlockHash
   * @throw new Error(s An exception on invalid sourceBlockHash
   */
   ...
   * @throw new Error(s An exception on invalid sourceBlockHash
   * @throw new Error(s An exception on invalid account
   * @throw new Error(s An exception on invalid representative account
   */
  api.setOpenParameters = function(
    sourceBlockHash,
    newAccount,
    ...
    try {
      account = functions.keyFromAccount(newAccount);
    } catch (err) {
    ...
    if (representativeAccount) {
      try {
        representative = functions.keyFromAccount(representativeAccount);
      } catch (err) {
   ...
   * @throw new Error(s An exception if work is not enough
   */
  ...
  api.getDestination = function() {
    if (type === "send") return functions.accountFromHexKey(destination);
  ...
  api.getHash = function(hex = false) {
    return hex ? hash : functions.hex_uint8(hash);
  };
  ...
  api.getBalance = function(format = "dec") {
    if (format === "dec") {
      var dec = bigInt(functions.hex2dec(balance));
      return dec;
    }
    return balance;
  };
  ...
  api.getRepresentative = function() {
    if (type === "change" || type === "open")
      return functions.accountFromHexKey(representative);
    else return false;
  };
   ...
   * @throw new Error(s An exception if its an open block
   * @throw new Error(s An exception if block is not built
   */
    ...
    switch (type) {
      ...
      case "send":
        api.setSendParameters(
          newPrevious,
          destination,
          functions.stringFromHex(balance).replace(RAI_TO_RAW, "")
        );
        api.build();
        break;
      case "change":
  ...
  api.getJSONBlock = function(pretty = false) {
    ...
    switch (type) {
      case "send":
        ...
        obj.destination = functions.accountFromHexKey(destination);
        obj.balance = balance;
        break;
      ...
      case "open":
        obj.source = source;
        obj.representative = functions.accountFromHexKey(
          representative ? representative : account
        );
        obj.account = functions.accountFromHexKey(account);
        break;

      case "change":
        obj.previous = previous;
        obj.representative = functions.accountFromHexKey(representative);
        break;
  ...
  api.getEntireJSON = function() {
    ...
    if (blockAmount) extras.blockAmount = blockAmount.toString();
    else extras.blockAmount = 0;
  ...
  api.buildFromJSON = function(json, v = false) {
    ...
    switch (obj.type) {
      case "send":
        ...
        destination = functions.keyFromAccount(obj.destination);
        balance = obj.balance;
        break;
      ...
      case "open":
        type = "open";
        source = obj.source;
        representative = functions.keyFromAccount(obj.representative);
        account = functions.keyFromAccount(obj.account);
        break;
